...
const ATTENDANCE_QUERY_KEYS = {
  all: ['attendance'],
  calendar: () => [...ATTENDANCE_QUERY_KEYS.all, 'calendar'],
  calendarMonth: (month) => [...ATTENDANCE_QUERY_KEYS.calendar(), month],
  stats: () => [...ATTENDANCE_QUERY_KEYS.all, 'stats'],
  statsMonth: (month) => [...ATTENDANCE_QUERY_KEYS.stats(), month],
  check: () => [...ATTENDANCE_QUERY_KEYS.all, 'check'],
};
 ...
 */
export function useAttendanceCalendar(month) {
  // 현재 월을 기본값으로 설정
 ...
 * 출석 통계 조회 훅
 */
export function useAttendanceStats(month) {
  const currentMonth = month || (() => {
    const now = new Date();
    return `${now.getFullYear()}-${(now.getMonth() + 1).toString().padStart(2, '0')}`;
  })();
 ...
 * 출석 달성도 조회 훅
 */
export function useAttendanceAchievement(month) {
  const currentMonth = month || (() => {
    const now = new Date();
    return `${now.getFullYear()}-${(now.getMonth() + 1).toString().padStart(2, '0')}`;
  })();
 ...
 */
export function useAttendanceDashboard(month) {

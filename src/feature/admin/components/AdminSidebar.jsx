...
export default function AdminSidebar({ currentPath }) {
  ...
  return (
    <aside
      className={`w-48 transition-transform ${isOpen ? "translate-x-0" : "-translate-x-full"
        } sm:translate-x-0`}
    >
        ...
        {menuItems.map((item) => {
          ...
          return (
            <li key={item.path}>
              <Link
                to={item.path}
                className={`flex items-center justify-center w-full my-2 py-3 rounded-full text-lg transition-colors ${isActive

...
import { validateEmail, validateVerificationCode } from '../../../shared/utils/validation';
...
export function usePasswordReset() {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      // Next.js router.push does not support 'state' object directly like react-router-dom.
      // For passing data like email and resetToken, query parameters or a global state management
      // solution (like Redux, Zustand, or React Context) should be used.
      // For this change, we'll use query parameters as a common approach.
      router.push(`${SIGNIN_CONSTANTS.ROUTES.RESET_PASSWORD}?email=${encodeURIComponent(emailValidation.email)}&resetToken=${encodeURIComponent(resetToken)}`);
    } catch (e: any) {
      setServerError(e.message);
    } finally {
  ...
  const onKeyDownInCode = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      e.preventDefault();
      handleVerifyCode();
    }

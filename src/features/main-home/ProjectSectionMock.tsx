...
export default function ProjectSectionMock() {
  ...
  const [openFilter, setOpenFilter] = useState<string | null>(null); // 'tech' | 'position' | null
  ...
  // 필터 옵션
  const techOptions = ["React", "Java", "Spring", "Node", "Python"];
            ...
            <div className="relative">
              ...
              {openFilter === 'tech' && (
                <div className="absolute top-full left-0 mt-1 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-20 py-1">
                  <button 
                    onClick={() => handleFilterChange('tech', null)}
                  ...
                  {techOptions.map(option => (
                    <button
                      key={option}
                      onClick={() => handleFilterChange('tech', option)}
              ...
              {openFilter === 'position' && (
                <div className="absolute top-full left-0 mt-1 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-20 py-1">
                  ...
                  {positionOptions.map(option => (
                    <button
                      key={option}
                      onClick={() => handleFilterChange('position', option)}

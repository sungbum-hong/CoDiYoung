...
export function useUpdateNickname() {
  ...
  return useMutation({
    ...
    onMutate: async (newNickname) => {
      ...
      queryClient.setQueryData(PROFILE_QUERY_KEYS.detail(), (oldData: any) => ({
        ...oldData,
...
export function useProfileOperations() {
  ...
  // 액션 함수들
  const changeNickname = useCallback(async (nickname: string) => {
  ...
  const changeProfileImage = useCallback(async (imageKey: string) => {
...
export function useProfileImageUpload() {
  ...
  const uploadAndUpdateImage = useCallback(async (file: File) => {
    try {
      ...
      throw new Error(`프로필 이미지 업로드 실패: ${(error as Error).message}`);
    }
  }, [updateImage]);

...
export default function ApplicantCard({ applicants, projectId, onApprove, onReject }: { applicants: any[]; projectId: any; onApprove?: (applicant: any) => void; onReject?: (applicant: any) => void }) {
  ...
  const handleApprove = async () => {
    ...
    try {
      // TODO: Implement ProjectService.approveApplicant
  ...
  const handleReject = async () => {
    ...
    setIsProcessing(true);
    try {
      // TODO: Implement ProjectService.rejectApplicant
    ...
    <div className="w-full max-w-4xl mx-auto p-4 h-full">
      ...
      <div className="border-2 border-purple-400 rounded-lg p-8 flex flex-col gap-6 h-full min-h-[600px]">
        ...
        <div className="w-full space-y-6 mt-8 flex-1">
          ...
          {/* 추가 질문들이 있다면 표시 */}
          {answers.slice(1).map((answer: any, index: number) => (
            <div key={index + 1}>

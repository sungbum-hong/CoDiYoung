...
export class ProjectUtils {
   ...
   */
  static isProjectLeader(user: any, project: any) {
   ...
   */
  static getCapacityInfo(project: any) {
   ...
   * 신청자 관련 정보 계산
   */
  static getApplicantInfo(applicants: any[], capacityInfo: any) {
   ...
   * 프로젝트 완료 진행률 계산
   */
  static getCompletionProgress(project: any) {
   ...
   * 완료 버튼 접근 권한 확인
   */
  static canAccessCompleteButton(user: any, project: any, appliedProjects: any[], isAuthenticated: boolean) {
    if (!isAuthenticated) return false;

    const projectId = project.id;
    const isApplicant = appliedProjects && appliedProjects.length > 0 &&
      appliedProjects.some((applied: any) => applied.id === projectId);
   ...
   * 완료 처리 결과 메시지 생성
   */
  static generateCompletionMessage(result: any) {
    if (!result?.success) {
   ...
   * 에러 메시지 정규화
   */
  static normalizeErrorMessage(error: any) {
   ...
   */
  static resolveImageUrl(key: string | null | undefined) {
   ...
   */
  static getMemberImageUrl(member: any) {
   ...
   */
  static getMemberDisplayName(member: any) {
   ...
   */
  static getDisplayMembers(project: any, options: any = {}) {
